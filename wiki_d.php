    ...
    echo '<div class="card" class="wikiDiscussCard">
            ...
            while($row = mysqli_fetch_assoc($discuss)){
                ...
                echo '<div class="card" style="border: 2px dashed gainsboro" class="wikiDiscussCard">
                    ...
                    <section>';
                    $sql = "SELECT * FROM `_discussThread` WHERE `origin` = '".$row['num']."' ORDER BY `at` LIMIT 5";
                    $result = mysqli_query($conn, $sql);
                    $d = 1;
                    while($dThread = mysqli_fetch_assoc($result)){
                        ...
                        $content = nl2br(documentRender($dThread['content']));
                    ...
                    if($count['cnt'] > 5 && $d >= 5){
                        ...
                        if($isActive){
                            if($canEdit || $id){
                            echo '<br><div class="card" class="wikiDiscussCard">
                                <header class="black noGray" style="background:#f6f6f6;border-bottom:1px solid #e6e6e6">
                                ...
                                <form action="/wiki_d.php?mode=speak&title='.$fnwTitle.'" method="post">
                                    <textarea name="content" placeholder="예의를 지킵시다. (1000자 이내 작성)" style="border:0" maxlength="1000" required></textarea>
